...
import { requireCenterAdmin, verifyCenterAccess } from '@/lib/api-auth'
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Require authentication - must be center_admin or super_admin
    const authResult = await requireCenterAdmin(request)
    ...
    // Verify user has access to this center's payment
    if (!verifyCenterAccess(authenticatedUser, payment.center_id)) {
      return NextResponse.json(
