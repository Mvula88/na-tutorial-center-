...
async function batchLoadRecipients(
  ...
  const teacherIds = notifications.filter(n => n.recipient_type === 'teacher').map(n => n.recipient_id)
  ...
  const [parents, students, teachers] = await Promise.all([
    ...
    studentIds.length > 0
      ...
      : { data: [] },
    teacherIds.length > 0
      ? supabase.from('teachers').select('id, email, phone, full_name').in('id', teacherIds)
      : { data: [] },
  ])
  ...
  for (const teacher of (teachers.data || []) as RecipientWithId[]) {
    recipientCache.set(`teacher:${teacher.id}`, { email: teacher.email, phone: teacher.phone, full_name: teacher.full_name })
  }
}
